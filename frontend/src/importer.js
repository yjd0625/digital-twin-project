...
export function initImporter(ctx) {
  ...
  // ======================== 默认变换管理（内存基线 + 复位）=======================
  /**
   * 捕获实例（可传单个 root 或数组）的默认局部变换到 userData._default。
   * 必须在模型刚加载、尚未被后端/用户改动时调用。
   * @param {THREE.Object3D | THREE.Object3D[]} rootOrList - 单个模型或模型数组
   */
  function captureDefaults(rootOrList) {
  ...
  /** 捕获所有已加载实例的默认基线（初始加载完成后调用一次）*/
  function saveDefaultTransforms() {
    captureDefaults(allModelInstances);
  }
