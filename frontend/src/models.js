...
/** 模块级缓存的 GLTFLoader 实例 */
let _cachedLoader = null;

/** 获取缓存的 GLTF 加载器（避免重复 import 和重复创建 DRACOLoader） */
async function getCachedGLTFLoader() {
...
export async function loadGLTFTemplate(url) {
  const loader = await getCachedGLTFLoader();
...
export async function loadDXFModel(scene, url, options = {}) {
  ...
  // 透明点击面（方便选中）
  const pw = Math.max(mx - nx || 1, 1), ph = Math.max(my - ny || 1, 1);
  const pgeo = new THREE.PlaneGeometry(pw, ph);
  const pmat = new THREE.MeshBasicMaterial({ transparent:true, opacity:0.005, side:THREE.DoubleSide });
  const cp = new THREE.Mesh(pgeo, pmat); cp.position.set((mx+nx)/2, (my+ny)/2, 0); group.add(cp);
  ...
  // 标签
  if (options.label) {
    const div = document.createElement("div"); div.textContent = options.label;
    ...
    const lbl = new CSS2DObject(div); lbl.position.set(0, size.y / 2 + 0.5, 0); group.add(lbl);
  }
